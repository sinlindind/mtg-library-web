...
export default function App() {
  ...
  // Load user card quantities from Supabase on start
  useEffect(() => {
  ...
  const fetchLibraryQuantities = async () => {
    ...
    // Build lookup map: { [scryfall_id]: { reg: X, foil: Y } }
    const qtyMap = {};
    (data || []).forEach((item) => {
  ...
  const handleAddCard = async (card, isFoil) => {
    ...
    // Upsert directly into Supabase user_cards table
    ...
    if (!error) {
      // Instant UI state update without full page re-renders
      setLibraryMap((prev) => ({
        ...prev,
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 font-sans">
      <h1 className="text-3xl font-bold mb-6 text-slate-800">MTG Library Search</h1>
      ...
      <form onSubmit={handleSearch} className="mb-8 flex gap-3">
        <input
          ...
          className="flex-1 p-3 border rounded-lg border-slate-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button 
          type="submit" 
          disabled={loading}
      ...
      <div className="space-y-4">
        {searchResults.map((card) => {
          ...
          return (
            <div key={card.id} className="flex gap-4 p-4 border border-slate-200 rounded-xl bg-white shadow-sm items-center">
              {imgUrl ? (
                <img src={imgUrl} alt={card.name} className="w-24 rounded-lg shadow-sm" />
              ) : (
                <div className="w-24 h-36 bg-slate-100 rounded-lg flex items-center justify-center text-xs text-slate-400">No Image</div>
              )}
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 text-sm">
                  {totalOwned > 0 ? (
                    <span className="inline-block bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full font-medium border border-emerald-200">
                      📦 In Library: {totalOwned}x ({owned.reg} Reg | {owned.foil} Foil)
                    </span>
                  ) : (
                    <span className="text-slate-400">📦 In Library: 0x</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 rounded-lg text-sm font-medium transition-colors"
                >
                  ✨ Foil
                </button>
              </div>
